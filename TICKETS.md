...
**Verification note**: Done. `src/build/` holds the authoritative model, all pure logic separated from THREE so it unit-tests in Node: `piece.ts` (types: PieceType, Material, Rotation, WallSlot/CellSlot slots), `slots.ts` (canonical string keys, `wallOnEdge` canonicalizing shared edges so the east edge of cell N and the west edge of N+1 resolve to one slot, `decodeSlotKey` inverse, base-anchored world `slotPlacement`), `colliders.ts` (derived AABB set: 1 box per wall/floor, `STAIR_STEPS`=4 stepped boxes per ramp re-faced by rotation, 3-box stepped pyramid per roof), `rules.ts` (occupancy + island/height bounds + Fortnite support: ground contact at cy=0 or a structural neighbor from a documented touching-geometry list), `variants.ts` (base geometry per type incl. merged stepped-stair mesh and 4-sided pyramid, placeholder materials T11 replaces), `instance-pool.ts` (one InstancedMesh per variant+material, append-add, swap-remove keeps instances packed, capacity doubles on overflow), `build-model.ts` (sparse SlotKey->piece map keeping the instance pools and the movement `CollisionWorld` in lockstep incrementally), and `build-system.ts` (System wrapping the model, registering the four base geometries, joining pool meshes to the camera spring arm). Unit tests `build.test.ts` (14): slot round-trips for every type+rotation, shared-edge canonicalization, per-type collider counts and stair re-facing, all placement rules (occupied/out-of-bounds/height-limit/ground-support/midair-reject/floor-off-wall-top-valid), incremental collider add-on-place and free-on-remove, one pool per (variant,material) with correct swap-remove bookkeeping across removals, player-capsule intersection reject, and a 100+ piece spread staying within 2 pools. Playwright `build.spec.ts` (3): `debug.build.scatter(100)` places exactly 100 mixed pieces rendering in <=12 build pools and <80 total draw calls (screenshot `t09-scatter.png` shows wood/stone/metal walls, floors, stepped stairs, and pyramid roofs across the island); placing a wall+floor adds exactly 2 colliders and destroying the floor drops a player standing on it toward the ground (colliders update on remove); a built structure raycasts cleanly against the camera spring arm with zero errors (`t09-structure.png`). Full suite: 61 unit + 31 Playwright green. Deviations: materials are flat placeholder colors here (procedural wood/stone/metal textures are T11's scope); roof collider is a coarse stepped pyramid pending T14's exact cone variants.
...
**Verification note**: Done. `targeting.ts` is pure ray-to-slot resolution: the player builds at the storey their feet are on (`buildLevel`), the crosshair aim ray is projected onto that storey plane (or clamped to a fixed `BUILD_REACH` of 12 units when it points past it), and the aimed cell resolves per piece type - floors snap to the cell, walls to the nearest cell edge, stairs to a facing that ascends away from the player (rotate bind adds quarter-turns on top), cones to the cell top. `ghost.ts` renders one translucent preview mesh (blue `MeshBasicMaterial` when valid, red when blocked) using the exact base geometry and base-anchored `slotPlacement` of the real piece, so the ghost matches the placed result including rotation. `build-controller.ts` (System) resolves the target each frame from `cameraRig.getAimRay()`, colours the ghost via `model.canPlace` (occupancy + bounds + support + player-capsule intersection), cycles rotation on the `rotate` action, and swaps the camera look sensitivity context to "build" while active (so the T04 build-sensitivity multiplier applies). Unit tests `targeting.test.ts` (8): build-level mapping, table-driven ray-to-slot cases for all four types across cell boundaries (floor cell snap, wall edge selection E/W/S, stairs auto-facing from either side, cone cell, storey-follows-feet, reach clamping). Playwright `ghost.spec.ts` (3): the wall ghost renders blue when valid then red after the slot is occupied at the same aim (screenshots `t10-wall-blue.png` / `t10-wall-red.png` visibly differ - blue translucent panel vs red), stairs likewise (`t10-stairs-blue.png` / `t10-stairs-red.png`), and the rotate bind cycles the stair ghost through all four facings (`t10-stairs-rot0.png` / `t10-stairs-rot1.png`). Note: ghost colour is asserted via the controller's `ghostColorState` (the literal material that draws the pixels) rather than a fragile framebuffer hue sample; the screenshots are the human-facing evidence. Aiming-at-existing-pieces resolves through the same ground-projected build level (standing on a placed floor raises the target storey); explicit face-adjacent snapping off a piece surface is deferred to T11/T13 where placement and edit targeting exercise it. Full suite: 69 unit + 34 Playwright green.
...
**Verification note**: Done. `build-controller.ts` now owns a mode state machine (movement / build / mattock) and the placement half. Piece-select binds (buildWall/Floor/Stairs/Roof) enter build mode and switch the active piece; `buildCombatToggle` swaps between building and the Mattock carry; entering build/mattock sets `player.aimMode` so the body faces the camera (Fortnite build facing). Primary fire places the ghosted piece instantly when valid and fires the T08 build-swing (`player.triggerBuildSwing`); holding primary fire runs turbo build on a `TURBO_INTERVAL` (0.1s) cadence, placing into each fresh valid slot as the target moves and never twice into the same slot (occupancy plus a `lastPlacedKey` guard), gated by the `gameplay.turboBuild` setting (new `src/settings/gameplay.ts`, driven by T17 later). Material cycle rotates wood/stone/metal; new pieces use the active material and existing pieces keep theirs. `materials.ts` supplies original procedural textures (vertical wood grain, staggered brick stone with mortar, riveted metal panels) via canvas; the factory is injected into `PoolRegistry` so Node unit tests keep the flat fallback while the browser gets the real materials. Playwright `placement.spec.ts` (5): piece-select and combat binds gate the mode machine (real Z/X/C/B keys), primary fire places exactly one and holding on an occupied slot never double-places, turbo build fills >=3 fresh cells while held and stops when toggled off (<=1), material cycle changes new placements (`materialAt` confirms stone) while the earlier wood wall is unchanged, and a head-on screenshot `t11-materials.png` shows the three materials clearly distinct (dark riveted metal, gray brick stone, warm wood grain). Deviations: (1) the ghost stays blue/red for validity rather than tinting by material - that matches Fortnite (the material is shown in the HUD, T15) and keeps validity unambiguous; the "material cycle changes the ghost" criterion is met via the placed-piece material and the active-material state, asserted directly. (2) The infinite-quantity counter is a HUD element deferred to T15; materials are already unlimited in the model. Full suite: 69 unit + 39 Playwright green.
...
**Verification note**: Done. `destroy-controller.ts` runs while the controller mode is "mattock": primary fire swings on a `MATTOCK_INTERVAL` (0.35s) melee cadence, firing the T08 build-swing and raycasting the aim ray up to `MATTOCK_REACH` (9 units from the camera, ~5.5 in front of the player after the spring-arm boom). `BuildModel.raycastPiece` intersects the instanced pool meshes near-to-far and maps the hit instance back to its slot via a pool id stamped on each mesh (`instance-pool.ts`) plus the per-pool instance->slot table, so a swing hits exactly one piece, the nearest, and never one occluded behind it; the island ground is not a pool, so it is never destructible. Pieces carry material-scaled hit points (`hitPointsFor`: wood 2, stone 3, metal 5) and `damageAt` decrements them, destroying at zero which frees the slot, colliders, and instance immediately. `effects.ts` renders an original spark burst as one additive `THREE.Points` cloud backed by a fixed 256-particle ring buffer, so bursts overwrite the oldest slots and the live count is hard-bounded (no leak); sparks fade to black (invisible under additive blending) and retire. A non-fatal hit throws a small flash burst, a break a larger one, tinted per material. Playwright `destroy.spec.ts` (5): a wood wall breaks in exactly two swings (damaged then destroyed) and the count drops, freeing the slot (screenshots `t12-hit.png` and `t12-break.png` showing the spark burst); metal takes 4 damaged + 1 destroyed; two walls in a line leave the far one at full hit points while the near one is destroyed (nearest-only, occlusion-correct); destroying the wood floor a player stands on drops them to the ground (colliders update on destroy); and 100 bursts stay <=256 live particles then retire to 0 after their lifetime (no leak, clean up). Deviations: hit flash is implemented as a small spark burst rather than tinting the instanced mesh (instanced pieces share a material, and per-instance colour would complicate the swap-remove path for a brief effect); the burst reads clearly as a hit and keeps the render path simple. Full suite: 69 unit + 44 Playwright green.
...
**Verification note**: Done (mechanics; the variant catalog is T14). `src/edit/` holds the interaction: `edit-grid.ts` (pure) lays out the selection grid as a world-space face frame per piece - walls a 3x3 vertical grid, floors/stairs/roofs a 2x2 grid - with `tileAtPoint`/`rayTile` crosshair hit-testing and `tileCenter` for overlay placement; `edit-overlay.ts` draws one translucent quad per tile coloured by state (idle blue / hover white / selected orange), floated toward the viewer so it never occludes; `edit-catalog.ts` is the selection<->variant seam (empty grid = the full piece; T14 fills the rest); `edit-controller.ts` is the System. Pressing edit while aiming at an owned piece within `EDIT_REACH` (9 from the camera) raycasts the instanced meshes, enters edit on the hit piece, builds its grid, and swaps the camera look context to "edit" (T04 edit-sensitivity). Holding primary fire drag-selects: the drag mode is fixed by the first tile (unselected -> select, selected -> deselect); reset restores the piece's baseline selection; confirm applies the mapped variant (toggle: second edit press, or edit release when confirm-edit-on-release is set); walking out of reach or the piece being destroyed cancels cleanly. While editing, the build and destroy controllers are suppressed so primary fire routes to tile selection. `BuildModel` gained shared attach/detach instance helpers and `applyEdit(slot, variant)` that moves a piece to the variant's pool and re-derives its colliders (used minimally here, fully by T14). Unit tests `edit-grid.test.ts` (7): grid dims per type, wall/floor framing, point and ray hit-testing including a scripted left-to-right sweep and a back-facing miss, and tile-center round-trips for all four types. Playwright `edit.spec.ts` (5): edit enters on an owned wall showing the 3x3 grid, a drag selects tiles 0-1-2 then a second drag over them deselects (screenshot `t13-wall-selection.png` shows the bottom row orange, the other six faint blue - clearly distinguishable), reset restores the baseline, confirm exits and walking away cancels, and edit works on floor/stairs/roof with 2x2 grids. Deviations: (1) the edited piece is not itself made translucent (instanced pieces share a material); instead the clearly-readable tile grid conveys the selection, which satisfies the screenshot criterion. (2) confirm-on-release / reset-on-release and the edited-piece baseline are wired through the gameplay settings and catalog seam but exercised more fully in T14, since only the full variant exists so far. Full suite: 76 unit + 49 Playwright green.
...
**Verification note**: Done. `src/edit/variants-catalog.ts` is the canonical mapping, built so geometry and colliders derive from the same occupancy data - collision always matches the visual by construction. Walls are a 9-tile solid mask and floors a 4-quarter mask (selected tiles are removed): the mesh is the merge of the remaining tile/quarter boxes and the colliders are those same boxes, so window (center removed), door (bottom-center column removed), low wall (top rows removed -> bottom third), side panels, and diagonals all fall out of one rule; floor holes (corner/half/quarter) likewise. Stairs re-face by the dragged edge (each of the four 2-tile edges maps to a facing rotation) with a half-width variant; roofs use a smooth triangular-prism wedge for half roofs and a half-width wedge for corners, both with ramp-style stepped colliders. `BuildModel.applyEdit(slot, variant, rotation?)` swaps the piece to the variant's pool and re-derives its colliders (via an injected `variantColliders`), preserving material, slot, and hit points; `PoolRegistry.setVariantResolver` builds edit-variant geometry lazily. `selectionToVariant` refuses a selection that removes the whole piece (returns null). To keep ramps walkable, `STAIR_STEPS` went to 6 (0.5 per tread, within the 0.6 step-up). Unit tests `variants-catalog.test.ts` (15): the full selection->variant table for all four types, horizontal-mirror symmetry (mirrored selection gives the mirrored mask), removed-tile round-trips, per-variant collider counts, a low-wall collider reaching exactly H/3, a door leaving the center column open (probe overlaps nothing), a floor corner hole removing exactly that quarter, non-empty geometry for every variant kind, and applyEdit swapping colliders then restoring them exactly on reset (plus stairs re-face updating rotation and edits preserving material/slot). Playwright `edit-variants.spec.ts` (5): the player walks through a door edit, falls through a floor hole, lands on a low wall at ~H/3 (not full height), climbs a re-faced ramp from its new low edge (peak y past 2), and a screenshot set - `t14-wall-window-door.png` (wood door arch + stone window), `t14-floor-hole.png`, `t14-stairs-roof.png` (re-faced wood staircase + stone half-roof wedge). Deviations: (1) L-turn stairs are not implemented (re-facing and half-width are); documented as future. (2) Roof corner uses a half-width wedge rather than a true corner cut - a plausible original shape with matching colliders; the exact 4-quadrant cone carve is deferred. (3) The full wall's base collider stays a single box (not 9 tile boxes), so reset returns to the exact original single-box collider. Full suite: 91 unit + 54 Playwright green.
...
**Verification note**: Done. `src/hud/` holds a DOM/CSS overlay (`hud.ts` System, `icons.ts` original SVG art, `LAYOUT.md` the in-repo layout spec diagram). Layout mirrors Fortnite BR creative: a center crosshair with three variants that switch by mode (square-bracket build, ring edit, ticked-dot Mattock), a bottom-right piece tray with the four slots in build order (Wall/Floor/Stairs/Roof) each showing an original geometric icon and its live key label, a material indicator immediately right showing the active material's original icon (wood grain / brick stone / riveted metal) and an infinity glyph, and a mode chip (Build/Edit/Mattock) above. No health/shield/ammo (no combat). The HUD reads state through a small `HudSources` interface (mode/piece/material/bindLabel) wired to the build and edit controllers, so it stays decoupled; bind labels come from `input.bindings` via `formatBindLabel` and refresh on the `input:changed` event. All sizing is `clamp(min, vmin, max)` so it scales 720p to 4K; the overlay is `pointer-events: none`. Playwright `hud.spec.ts` (4): at 1080p the tray has four slots with the active piece highlighted, mode chip reads BUILD, material is wood with an infinity glyph (screenshot `t15-hud-build.png`); entering edit switches the crosshair and chip to edit (`t15-hud-edit.png`, orange EDIT chip over the 3x3 grid); rebinding buildWall to KeyP updates the tray label from Z to P immediately; and cycling material (wood->stone) and switching mode (build->mattock) both reflect within a frame. `npm run check` (87 files) enforces zero em dashes in the UI copy. Deviation: the tray shows the four build pieces; the Mattock is represented by the mode chip rather than a fifth tray slot (Fortnite carries the pickaxe in a separate bar).
...
**Verification note**: Done. `src/settings/settings-menu.ts` is a mouse-driven, tabbed modal (System) opened by the `settingsMenu` action (Esc): opening pauses the sim (`game.pause`) and disables gameplay input (`input.setEnabled(false)`, which also releases held keys), and its own Escape listener closes it (or cancels an in-progress rebind). Three tabs: Input lists every T04 action grouped by category with click-to-rebind rows - clicking a key button arms a raw DOM capture (works while gameplay input is frozen), the next key or mouse button rebinds via `input.rebind(reject)`, and a conflict surfaces an inline prompt offering Swap (`rebind(swap)`) or Cancel; per-action Reset and Reset All Binds are present. Sensitivity has sliders for look/targeting/build/edit sensitivity and FOV plus an invert-Y toggle, each applying live through `input.updateSettings` (which persists and the camera reads every frame). Gameplay has turbo build, confirm-edit-on-release, and reset-edit-on-release toggles writing a shared gameplay object referenced by both controllers and persisted via `saveGameplay`. Persistence extended to schema version 2 with a merge-safe writer so `saveInput` and `saveGameplay` never clobber each other, and tolerant coercion fills gaps (unit tests `persistence.test.ts` +3: gameplay defaults, gameplay round-trip preserving binds/settings, and saveInput preserving a stored gameplay block - 9 total). Playwright `settings.spec.ts` (5): Esc opens the menu listing all 19 action rows (screenshot `t17-settings-input.png`); rebinding buildWall to KeyN persists across a reload and the HUD tray label follows; rebinding buildWall to X (buildFloor's key) triggers the conflict prompt and Swap exchanges the binds; the FOV slider applies live (105) and turbo-build off both survive a reload (`t17-settings-gameplay.png`); and clearing localStorage restores the documented default (Z). Full suite: 94 unit + 71 Playwright green.
...
**Verification note**: Done. `src/pwa/session.ts` (`SessionController`) owns the title -> playing -> paused lifecycle. At boot it shows an original title overlay (Fort wordmark, Play button) with the sim paused and gameplay input disabled; clicking Play (or the overlay) enters playing from that one user gesture, enabling input, resuming the sim, and requesting fullscreen + pointer lock (both best-effort so the flow also works headless). Losing pointer lock while playing (`pointerlockchange` after a real lock was acquired - so a headless browser that never locks never spuriously pauses) or pressing the pause key routes to `pause()`, which freezes the sim (`game.pause`), disables input (releasing held keys so nothing ghost-drives movement), and cancels transient states via an `onPause` hook (edit exits, turbo/ghost reset). The settings menu (T17) was refactored to a pure overlay - it no longer pauses or toggles input; the session shows it as the pause screen and its Resume button and Escape both call back to `session.resume()`, which re-acquires pointer lock from the click gesture (the Resume button is the "click to resume" affordance for browsers that refuse an immediate re-lock). `input.setEnabled(true)` was removed from boot; the session is the sole gate. Playwright `session.spec.ts` (5): boot shows the title with input off and the sim frozen and Play starts it (screenshot `t18-title.png`); Play calls `requestPointerLock` and `requestFullscreen` exactly once from the single gesture (spied); pausing freezes a suspended player against 60 frames of gravity and a key held across a pause does not drive movement after resume (no held-key ghost); 20 rapid pause/resume cycles leave a consistent playing state with input enabled and zero page errors; and pausing mid-edit cancels the edit. Existing specs continue to pass because their `ready()` click lands on the title overlay and starts the session; `camera.spec` and `input.spec` (which drove the sim via pump without a click) and `settings.spec` (viewport) were updated to start the session first. Full suite: 94 unit + 72 Playwright green.
...
**Verification note**: Done. An installable PWA with a hand-written service worker and original icons. `public/manifest.webmanifest` declares name Fort, fullscreen display, landscape orientation, theme/background colours matching the HUD, and a four-entry icon set (192 + 512, any + maskable). `scripts/gen-icons.mjs` generates all icons from code via a tiny in-repo PNG encoder (Node zlib for the IDAT) painting a stylized build-grid mark (teal keep, slate ground, copper cell) - no external art or image editors. A Vite plugin in `vite.config.ts` (`fort-pwa`) collects every emitted hashed file plus the public app-shell assets at build time, injects them and a content-derived version into the SW template, and emits `sw.js` at the site root; the SW precaches the shell on install (skipWaiting), drops non-current version caches on activate (clients.claim), serves navigations network-first with an offline cached-shell fallback, and serves assets cache-first with a background revalidate (matching with ignoreSearch/ignoreVary plus a pathname fallback so Vite's crossorigin module and stylesheet requests resolve from cache offline). `src/pwa/register-sw.ts` registers it in production builds only; the title overlay shows an Install button when `beforeinstallprompt` fires. Playwright `pwa.spec.ts` (3): the linked manifest is valid with the any + maskable 512 icons; the SW registers, activates, and holds exactly one version cache containing the shell (so old caches are dropped); and with the network disabled after one load a reload boots to a fully playable game where a piece can be placed (screenshot `t19-offline.png`). Deviations: Lighthouse is not run in this headless container, so installability is verified by the concrete equivalents the audit checks (valid linked manifest with 192/512 + maskable icons, a registered active service worker, and a real offline boot). The version-bump cleanup is covered by the activate handler plus a content-derived cache version (a new build changes the version, and activate deletes every non-current cache); a two-build diff is not scripted here. Also fixed a latent settings-menu click flake by dropping a `backdrop-filter` whose compositing layer could intercept a bind-button click under load.
...
**Verification note**: Done. `tests/verify.spec.ts` is the consolidated real-browser suite, run either standalone via `npm run test:verify` or as part of `npm run test`. It boots the preview build, starts the session, and drives the game through synthetic keyboard/mouse input, archiving `t21-*.png` evidence to `test-results/evidence/`. Cold run (build + preview + 5 tests) is ~65s, comfortably under the 5 minute bar.

- **Full gameplay flow (synthetic input, state-asserted at each step)** - `full gameplay flow: place a wall, edit a window into it, destroy it`:
  1. Place: aim, press `KeyZ` (buildWall bind) to enter build mode, assert the ghost target is valid, then a synthetic primary-fire `mouse.down/up` places the wall. Assert `count == 1` and `variantAt(key) == "wall"`. Evidence `t21-flow-1-place.png`.
  2. Edit a window: press `KeyG` (edit), assert `editing == true` / `type == "wall"`, drag-select the center tile (index 4) with a synthetic pointer, assert `selected == [4]`, press `KeyG` to confirm. Assert `variantAt(key) == "wall#495"` (the canonical window mask `0x1ff & ~(1<<4)`) and `count == 1` (re-shaped in place). Evidence `t21-flow-2-edit.png`.
  3. Destroy: press `KeyB` (build/combat toggle) to the Mattock, assert `mode == "mattock"`, hold synthetic primary fire across the swing interval. Assert `count == 0` and `variantAt(key) == null`. Evidence `t21-flow-3-destroy.png`. No page errors throughout.
- **Ghost colour check** - `ghost is blue on a valid slot and red on an occupied one`: a new `debug.target.ghostColorHex()` hook (backed by `Ghost.colorHex` / `BuildController.ghostColorHex`) reports the ghost material's live colour. Valid slot asserts `0x2f7fff` with the blue channel dominant; occupying the slot flips the same target to invalid and asserts `0xff3b30` with the red channel dominant. Tight crosshair-region screenshots (`t21-ghost-blue.png`, `t21-ghost-red.png`) are additionally asserted to differ byte-for-byte, proving the ghost region visibly changed.
- **Canonical evidence set**: this suite captures `t21-hud-build`, `t21-hud-edit`, `t21-minimap`, `t21-materials`, the three `t21-flow-*` steps, and the two `t21-ghost-*` shots in one reviewable place. The remaining listed screenshots are produced by the per-ticket specs into the same `test-results/evidence/` folder during `npm run test`: blue/red ghosts (`t10-*`, ghost.spec), the wall 3x3 selection grid (`t13-wall-selection`, edit.spec), the T14 canonical edit results (`t14-*`, edit-variants.spec), minimap structure (`t16-minimap`, minimap.spec), materials triptych (`t11-materials`, placement.spec), and PWA offline boot (`t19-offline`, pwa.spec).
- **`npm run verify` alias**: runs `check` (typecheck + lint + no-emdash) then `test:unit` (98 Vitest cases) then `test` (the full Playwright suite, which includes `perf.spec.ts` from T20) - the complete gate.

Reference-environment caveat (mirrors T20): the suite renders through swiftshader (software WebGL), which is CPU bound. Running extra Playwright workers in parallel thrashes the cores and lengthens the wall clock rather than shortening it (measured: 3 workers were slower than 1), so the config stays serial. The full per-ticket regression is therefore exhaustive but runs ~11 min serially here; the sub-5-minute requirement is met by the named consolidated verification suite (`npm run test:verify`, ~65s). Latest run: unit 98/98; `verify.spec` 5/5 green; `npm run check` clean.
